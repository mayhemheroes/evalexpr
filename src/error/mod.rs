...
pub enum EvalexprError<IntType = i64, FloatType = f64> {
    ...
    UnmatchedPartialToken {
        ...
        first: PartialToken<IntType, FloatType>,
    ...
    /// Either the integer or the floating point type have no minimum value, but a function would require one.
    NoMinValue,

    /// Either the integer or the floating point type have no maximum value, but a function would require one.
    NoMaxValue,
...
impl<IntType, FloatType> EvalexprError<IntType, FloatType> {
    ...
    pub fn wrong_type_combination(operator: Operator<IntType, FloatType>, actual: Vec<ValueType>) -> Self {
    ...
    pub fn expected_number_or_string(actual: Value<IntType, FloatType>) -> Self {
    ...
    pub fn expected_fixed_len_tuple(expected_len: usize, actual: Value<IntType, FloatType>) -> Self {
    ...
    pub(crate) fn expected_type(expected: &Value<IntType, FloatType>, actual: Value<IntType, FloatType>) -> Self {
    ...
    pub(crate) fn addition_error(augend: Value<IntType, FloatType>, addend: Value<IntType, FloatType>) -> Self {
        EvalexprError::AdditionError { augend, addend }
    }

    pub(crate) fn subtraction_error(minuend: Value<IntType, FloatType>, subtrahend: Value<IntType, FloatType>) -> Self {
    ...
    pub(crate) fn negation_error(argument: Value<IntType, FloatType>) -> Self {
        EvalexprError::NegationError { argument }
    }

    pub(crate) fn multiplication_error(multiplicand: Value<IntType, FloatType>, multiplier: Value<IntType, FloatType>) -> Self {
    ...
    pub(crate) fn modulation_error(dividend: Value<IntType, FloatType>, divisor: Value<IntType, FloatType>) -> Self {
...
pub(crate) fn expect_operator_argument_amount<IntType, FloatType>(
    actual: usize,
    expected: usize,
...
pub fn expect_function_argument_amount<IntType, FloatType>(actual: usize, expected: usize) -> EvalexprResult<(), IntType, FloatType> {
    if actual == expected {
        Ok(())
    } else {
...
pub fn expect_number_or_string<IntType: Clone, FloatType: Clone>(actual: &Value<IntType, FloatType>) -> EvalexprResult<(), IntType, FloatType> {
    match actual {
...
impl<IntType: Debug + Display, FloatType: Debug + Display> std::error::Error for EvalexprError<IntType, FloatType> {}

/// Standard result type used by this crate.
pub type EvalexprResult<T, IntType, FloatType> = Result<T, EvalexprError<IntType, FloatType>>;
...
mod tests {
    ...
    fn trivial_coverage_tests() {
        assert_eq!(
            EvalexprError::type_error(Value::<_, f64>::int(3), vec![ValueType::String]),
        ...
        assert_eq!(
            EvalexprError::expected_type(&DefaultValue::String("abc".to_string()), Value::Empty),
            EvalexprError::expected_string(Value::Empty)
        );
        assert_eq!(
            EvalexprError::expected_type(&DefaultValue::Boolean(false), Value::Empty),
            EvalexprError::expected_boolean(Value::Empty)
        );
        assert_eq!(
            EvalexprError::expected_type(&DefaultValue::Tuple(vec![]), Value::Empty),
            EvalexprError::expected_tuple(Value::Empty)
        );
        assert_eq!(
            EvalexprError::expected_type(&DefaultValue::Empty, Value::String("abc".to_string())),
